    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            with torch.no_grad():
                ...
                z_ori = F.normalize(F.adaptive_avg_pool2d(feat_ori[-1], (1, 1)).squeeze(), dim=1)
            ...
            x_adv = Variable(inputs_u_s.detach(), requires_grad=True)
            _, feat_adv = model(x_adv, return_feature=True, adv=True)
            z_adv = F.normalize(F.adaptive_avg_pool2d(feat_adv[-1], (1, 1)).squeeze(), dim=1)
            l_ct_og = CL(z_ori.detach(), z_adv).mean()
            if args.amp:
            ...
            with torch.no_grad():
                ...
                x_adv.data = x_adv.data + args.eps * sign_grad
            inputs_adv = x_adv.detach()
            optimizer.zero_grad()

            logits, feats = model(all_x, return_feature=True)
            logits_x = logits[:batch_size]
            logits_u_w, logits_u_s = logits[batch_size:].chunk(2)
            feats_w = tuple(x[batch_size:8 * batch_size].detach() for x in feats)
            feats_s = tuple(x[8 * batch_size:].detach() for x in feats)
            del logits, feats
            ...
            with torch.no_grad():
                ...
                prec_unlab_strong = (targets_s == targets_ux).float()
