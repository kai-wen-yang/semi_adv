...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            mask_new = ((max_probs >= 0.65) & (max_probs <= 0.95)).float()
            ...
            if args.world_size > 1:
                mask_all = torch.cat(GatherLayer.apply(mask_new), dim=0)
                ...
                with torch.no_grad():
                    ...
                    if args.local_rank in [-1, 0]:
                        run.log({'loss/l_cs': l_cs.data.item(),
                                 ...
                                 'mask': mask.mean().item(),
                                 'mask_new': mask_new.mean().item(),
                                 'lr': optimizer.param_groups[0]['lr']})
            else:
                ##CDAA
                #random init
                eps = epsilon[index_selected] + args.step # bs
                ...
                with torch.no_grad():
                    ...
                    if args.local_rank in [-1, 0]:
                        run.log({'loss/l_cs': l_cs.data.item(),
                                 ...
                                 'Adv/ce_delta_his': wandb.Histogram((l_adv_ori-ce_ori).cpu().detach().numpy(), num_bins=512),
                                 'lr': optimizer.param_groups[0]['lr']})
                        if batch_idx == 1:
