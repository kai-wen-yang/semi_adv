...
def CL(x1_feat, x2_feat, world_size):
    x1_feat = F.adaptive_avg_pool2d(x1_feat, 1)
    x2_feat = F.adaptive_avg_pool2d(x2_feat, 1)
    x1_feat = x1_feat.view(-1, x1_feat.size(1))
    x2_feat = x2_feat.view(-1, x2_feat.size(1))
    x1_feat = x1_feat.div(x1_feat.pow(2).sum(1, keepdim=True).pow(1. / 2))
    x2_feat = x2_feat.div(x2_feat.pow(2).sum(1, keepdim=True).pow(1. / 2))

    if world_size>1:
        x1_feat = torch.cat(GatherLayer.apply(x1_feat), dim=0)
        x2_feat = torch.cat(GatherLayer.apply(x2_feat), dim=0)

    x1_x2_mat = torch.exp(torch.matmul(x1_feat, x2_feat.t())/0.1)
    denominator1 = torch.sum(x1_x2_mat, dim=1).view(x1_feat.size(0), 1)
    prob1 = x1_x2_mat / denominator1
    prob1_diag = torch.diag(prob1)
    l_ct = -(torch.log(prob1_diag))
...
def main():
    ...
    def run_iter(
        ...
        with torch.no_grad():
            _, feat_u_w = model(inputs_u_w, adv=True, return_feature=True)
        x_prior = Variable(inputs_u_w.detach(), requires_grad=True)

        _, feat_adv = model(x_prior, adv=True, return_feature=True)
        loss_tmp = CL(feat_u_w[-1], feat_adv[-1], args.world_size).mean()
        if args.amp:
        ...
        feat_u_w, _ = feat[-1][batch_size:].chunk(2)
        del logits, feat
        ...
        mask = torch.cat(GatherLayer.apply(mask), dim=0)
        l_adv = (CL(feat_u_w.detach(), feat_adv[-1], args.world_size) * mask).mean()/args.world_size
