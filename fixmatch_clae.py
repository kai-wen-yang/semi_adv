...
def main():
    ...
    parser.add_argument('--dataset', default='cifar10', type=str,
                        choices=['cifar10', 'cifar100', 'cifar10_v2'],
                        help='dataset name')
    ...
    def run_iter(
        ...
        x_adv.detach()
        logits_adv, feat_adv = model(x_adv.detach(), return_feature=True, adv=True)

        logits, feat = model(all_x, return_feature=True)
        logits_x = logits[:batch_size]
        logits_u_w, logits_u_s = logits[batch_size:].chunk(2)
        feat_u_w, feat_u_s = feat[batch_size:].chunk(2)
        del logits
        ...
        l_ct = (CL(feat_u_w.detach(), feat_adv) * mask).mean()
        loss = l_ce + l_cs + l_ct
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            l_ce, l_cs, mask = run_iter(inputs_x_w, targets_x, inputs_u_w, inputs_u_s, targets_ux)
