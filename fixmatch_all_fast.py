...
def main():
    ...
    adv_loader = Variable(torch.zeros([50000,3,32,32]),requires_grad=False)
    if args.use_ema:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            if unlabeled_epoch % 10 == 0:
                ...
                inputs_adv = get_attack(model, inputs_u_w, targets_uadv, y_w, flat_feat_ori, args)
                optimizer.zero_grad()
            else:
                inputs_adv = adv_loader[index.cpu()].to(args.device)
            ...
            mask_1 = max_probs.ge(args.our_threshold)
            ...
            with torch.no_grad():
                ...
                if batch_idx % 10 == 0:
                    if args.local_rank in [-1, 0]:
                        ...
                        prec_pesudo_adv = (targets_u == targets_adv)[mask_1].float().mean()
                        l2_norm = (inputs_adv - inputs_u_w).reshape(
                            inputs_u_w.shape[0], -1).norm(dim=1)[mask_1]
            ...
            if unlabeled_epoch % 10 == 0:
                ...
                with torch.no_grad():
                    if args.world_size > 1:
                        ...
                        inputs_adv_all = torch.cat(GatherLayer.apply(inputs_adv), dim=0)
                    else:
                        index_all = index
