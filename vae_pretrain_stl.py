...
def run_batch(x, y, model, optimizer, re):
    ...
    l_kl /= bs * 3 * args.dim
    loss = re * l_rec + args.kl * l_kl
    loss.backward()
    optimizer.step()

    return loss, l_rec, l_kl
...
def main(args):
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.4408, 0.4279, 0.386), (0.2682, 0.2610, 0.2686)),
    ])

    if (args.dataset == 'stl10'):
        print("| Preparing STL-10 dataset...")
        sys.stdout.write("| ")
        trainset = torchvision.datasets.STL10(root='../data', split='train+unlabeled', download=True, transform=transform_train)

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=True)

    # Model
    print('\n[Phase 2] : Model setup')
    model = SVAE_cifar_withbn(128, args.dim)
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        for batch_idx, (x, y) in enumerate(trainloader):
            bs = x.size(0)
            loss, l_rec, l_kl = \
                run_batch(x, y, model, optimizer, re)
