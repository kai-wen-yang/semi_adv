...
def main():
    ...
    parser.add_argument('--alpha', default=0.5, type=float, help='alpha for adversarial')
    ...
    parser.add_argument('--lmda', default=0.3, type=float, help='alpha for adversarial')
    ...
    optimizer_G = optim.SGD(G.parameters(), lr=args.lr,
                          momentum=0.9, nesterov=args.nesterov)
    ...
    def run_iter(
        ...
        logits_adv, feat_adv = model(G(inputs_u_w, args.lmda), adv=True, return_feature=True)
        ...
        with torch.no_grad():
            x_prior = G(inputs_u_w, args.lmda)
        ...
        loss = l_ce + (1-args.alpha) * l_cs + args.alpha * l_adv
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
